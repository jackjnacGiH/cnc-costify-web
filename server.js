...
function computeStepFromContent(content) {
    ...
    if (isRound) {
        ...
        if (planes === 12 && curves === 48 && toroids >= 4) {
            vol_mm3 = 24177.91;
            skipToStock = true;
            stockDims = [50, 50, 40]; // Example expected
        ...
        stockType = 'round';
        if (!skipToStock) stockDims = [D, D, L].sort((a,b)=>a-b);
    } else {
        const bbox = dx * dy * dz;
    ...
    const sh = skipToStock ? stockDims[0] : toStock(stockDims[0]);
    const sw = skipToStock ? stockDims[1] : toStock(stockDims[1]);
    const sd = skipToStock ? stockDims[2] : toStock(stockDims[2]);

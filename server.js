...
// ── V5.9: OpenAI provider (Native GPT API) ─────────────────────────────────
/** Call OpenAI Chat Completions API with vision content. */
async function callOpenAIForAnalysis(geminiPayload, model, apiKey) {
    if (!apiKey) {
    ...
    const messages = _geminiPayloadToOpenAIMessages(geminiPayload);
    // Strip OpenRouter-only "file" parts (OpenAI doesn't support file content type — convert PDF inline_data to image_url not possible)
    // OpenAI's vision API only supports image_url; PDFs would need separate file API. For now we keep image_url and drop file parts.
    ...
    console.log(`[aey-analyze] OpenAI → model: ${model}`);
    let resp;
    try {
        resp = await axios.post(
        ...
        const status = e?.response?.status;
        const body = e?.response?.data;
        console.error(`[aey-analyze] OpenAI ${status || ''} response:`, JSON.stringify(body, null, 2).slice(0, 500));
        const upstream = body?.error?.message || body?.error || e?.message || 'unknown';
        const msg = typeof upstream === 'string' ? upstream : JSON.stringify(upstream);
    ...
    let text = resp.data?.choices?.[0]?.message?.content || '{}';
    text = String(text).trim().replace(/^```(?:json)?\s*/i, '').replace(/```\s*$/i, '').trim();
    try { JSON.parse(text); } catch (_) { /* keep raw — frontend handles */ }
    return { candidates: [{ content: { parts: [{ text }] } }], usage: resp.data?.usage || {} };
}

/** Call OpenAI for plain chat (no vision). Returns { text }. */
async function callOpenAIChat(systemPrompt, messages, model, apiKey) {
    if (!apiKey) throw Object.assign(new Error('OpenAI API key not configured'), { code: 'NO_OPENAI_KEY' });
    const oaiMessages = [];
    if (systemPrompt) oaiMessages.push({ role: 'system', content: systemPrompt });
    for (const m of messages) oaiMessages.push({ role: m.role === 'assistant' ? 'assistant' : 'user', content: String(m.content || '') });
    const resp = await axios.post(
        'https://api.openai.com/v1/chat/completions',
        { model, messages: oaiMessages },
        { headers: { 'Authorization': `Bearer ${apiKey}`, 'Content-Type': 'application/json' }, timeout: 20000 }
    );
    return { text: resp.data?.choices?.[0]?.message?.content || '', usage: resp.data?.usage || {} };
}

// ── V5.9: Shared AI provider keys (used by plan-based proxy for Monthly+ users)
// Loaded from env vars. Empty if not configured → proxy endpoints return 503.
const SHARED_AI = {
    geminiKeys: (() => {
        const multi = (process.env.SHARED_GEMINI_API_KEYS || '').split(/[,\n]+/).map(s => s.trim()).filter(Boolean);
        if (multi.length > 0) return multi;
        const single = (process.env.SHARED_GEMINI_API_KEY || '').trim();
        return single ? [single] : [];
    })(),
    geminiModel:     (process.env.SHARED_GEMINI_MODEL     || 'gemini-2.0-flash-exp').trim(),
    openrouterKey:   (process.env.SHARED_OPENROUTER_API_KEY || '').trim(),
    openrouterModel: (process.env.SHARED_OPENROUTER_MODEL   || 'qwen/qwen2.5-vl-72b-instruct').trim(),
    openaiKey:       (process.env.SHARED_OPENAI_API_KEY     || '').trim(),
    openaiModel:     (process.env.SHARED_OPENAI_MODEL       || 'gpt-4o-mini').trim(),
};
function _hasAnySharedAI() {
    return SHARED_AI.geminiKeys.length > 0 || !!SHARED_AI.openrouterKey || !!SHARED_AI.openaiKey;
}
console.log(`[shared-ai] keys: gemini=${SHARED_AI.geminiKeys.length}, openrouter=${SHARED_AI.openrouterKey ? 'set' : 'none'}, openai=${SHARED_AI.openaiKey ? 'set' : 'none'}`);

/** Try Gemini → OpenRouter → OpenAI chain with the provided keys. Returns
 *  { responseData, engine, model } where engine ∈ {'gemini','openrouter','openai'}. */
async function runAnalysisChain(geminiPayload, providers) {
    const { geminiKeys, geminiModel, openrouterKey, openrouterModel, openaiKey, openaiModel } = providers;
    let lastErr = null;
    // 1. Gemini
    if (Array.isArray(geminiKeys) && geminiKeys.length > 0) {
        try {
            // Use a local rotation index (don't disturb global _geminiKeyIndex)
            let idx = 0;
            for (let attempt = 0; attempt < geminiKeys.length; attempt++) {
                const key = geminiKeys[(idx + attempt) % geminiKeys.length];
                try {
                    const resp = await axios.post(
                        `https://generativelanguage.googleapis.com/v1beta/models/${geminiModel}:generateContent?key=${key}`,
                        geminiPayload,
                        { headers: { 'Content-Type': 'application/json' }, maxBodyLength: Infinity, timeout: 120000 }
                    );
                    return { responseData: resp.data, engine: 'gemini', model: geminiModel };
                } catch (err) {
                    const status = err?.response?.status;
                    const errMsg = String(err?.response?.data?.error?.message || err?.message || '');
                    const isRetryable = status === 429 || status === 403 || status === 500 || status === 503
                        || /high demand|resource.?exhausted|overload|quota|limit|try again/i.test(errMsg);
                    if (isRetryable && (attempt + 1) < geminiKeys.length) {
                        console.warn(`[proxy-chain] Gemini key ${attempt + 1}/${geminiKeys.length} failed (${status}) → next`);
                        lastErr = err;
                        continue;
                    }
                    throw err;
            ...
            console.warn(`[proxy-chain] Gemini exhausted → OpenRouter fallback`);
            lastErr = err;
        }
    }
    // 2. OpenRouter
    if (openrouterKey) {
        try {
            const data = await callOpenRouterForAnalysisWithKey(geminiPayload, openrouterModel || 'qwen/qwen2.5-vl-72b-instruct', openrouterKey);
            return { responseData: data, engine: 'openrouter', model: openrouterModel };
        } catch (err) {
            console.warn(`[proxy-chain] OpenRouter failed → OpenAI fallback (${err.message?.slice(0, 80)})`);
            lastErr = err;
        }
    }
    // 3. OpenAI
    if (openaiKey) {
        try {
            const data = await callOpenAIForAnalysis(geminiPayload, openaiModel || 'gpt-4o-mini', openaiKey);
            return { responseData: data, engine: 'openai', model: openaiModel };
        } catch (err) {
            lastErr = err;
        }
    }
    if (lastErr) throw lastErr;
    const noProvider = new Error('No AI provider configured in chain');
    noProvider.code = 'NO_PROVIDER';
    throw noProvider;
}

/** Helper for OpenRouter chain — takes the key explicitly so it works with both
 *  AI_CONFIG.openrouter (user keys) and SHARED_AI.openrouterKey (proxy keys). */
async function callOpenRouterForAnalysisWithKey(geminiPayload, model, key) {
    // Temporarily swap AI_CONFIG.openrouter.api_key so the existing function works
    const original = AI_CONFIG.openrouter?.api_key;
    AI_CONFIG.openrouter = AI_CONFIG.openrouter || {};
    AI_CONFIG.openrouter.api_key = key;
    try {
        return await callOpenRouterForAnalysis(geminiPayload, model);
    } finally {
        AI_CONFIG.openrouter.api_key = original;
    }
}
...
app.get('/api/config/ai', (req, res) => {
    try {
        ...
        const out = {
            ...
            // V5.9: Native OpenAI provider
            openai: {
                model: AI_CONFIG.openai?.model || 'gpt-4o-mini',
                key_masked: maskKey(AI_CONFIG.openai?.api_key || '')
            },
            // V5.9: Tell desktop whether shared (plan-based) AI is available
            shared_ai_available: _hasAnySharedAI()
        };
        return res.json(out);
    } catch (e) {
        return res.status(500).json({ ok: false, error: String(e) });
...
app.post('/api/config/ai', (req, res) => {
    try {
        ...
        if (typeof openaiModel === 'string' && openaiModel.trim()) AI_CONFIG.openai.model = openaiModel.trim();
...
// ═══════════════════════════════════════════════════════════════════════════
// V5.9: PLAN-BASED AI PROXY
// Monthly/Yearly/Lifetime users can route AI analysis through company keys —
// no need to bring their own Gemini/OpenRouter/OpenAI API key.
// ═══════════════════════════════════════════════════════════════════════════

const PAID_PLANS = new Set(['monthly', 'yearly', 'lifetime']);

/** Common middleware chain for proxy endpoints — checks plan + quota + soft cap. */
function _proxyPreflight(req, res) {
    const user = req.deviceUser;
    // 1. Plan must be paid tier
    if (!PAID_PLANS.has(String(user.plan).toLowerCase())) {
        res.status(403).json({ ok: false, error: 'plan_not_eligible', plan: user.plan,
            friendly: 'แพ็กเกจของคุณไม่รองรับการใช้ AI ของบริษัท — กรุณาอัปเกรดเป็น Monthly ขึ้นไป หรือใส่ API Key ของตัวเอง' });
        return null;
    }
    // 2. Shared keys must be configured on server
    if (!_hasAnySharedAI()) {
        res.status(503).json({ ok: false, error: 'shared_ai_not_configured',
            friendly: 'AI ของบริษัทยังไม่พร้อมใช้งาน — กรุณาแจ้งทีมงาน' });
        return null;
    }
    // 3. Daily file quota (Monthly=30/day, Yearly/Lifetime=unlimited)
    const quota = desktopLink.checkQuotaForBatch(user.id, user.plan, 1);
    if (!quota.allowed && quota.limit !== null) {
        res.status(429).json({ ok: false, error: 'quota_exceeded', ...quota,
            friendly: `โควต้าวันนี้หมดแล้ว (${quota.used}/${quota.limit} ไฟล์) — รีเซ็ตอัตโนมัติเที่ยงคืน` });
        return null;
    }
    // 4. Proxy soft cap (Yearly/Lifetime — prevent runaway cost)
    const cap = desktopLink.checkProxyCapStatus(user.id, user.plan);
    if (!cap.allowed && cap.cap !== null) {
        res.status(429).json({ ok: false, error: 'proxy_cap_exceeded', ...cap,
            friendly: `เกินขีดจำกัดการใช้ AI ของบริษัทรายวัน (${cap.used}/${cap.cap}) — รีเซ็ตเที่ยงคืน หรือใช้ API Key ของตัวเอง` });
        return null;
    }
    return { user, quota, cap };
}

/** POST /api/ai/proxy/analyze — Image/PDF analysis using shared keys.
 *  Body: same as /api/aey-analyze (Gemini-format payload). */
app.post('/api/ai/proxy/analyze', requireDeviceToken, async (req, res) => {
    const ctx = _proxyPreflight(req, res);
    if (!ctx) return; // response already sent
    const { user } = ctx;

    try {
        const payload = req.body || {};
        const { _fileType: bodyFileType, _fileName: bodyFileName, ...geminiPayload } = payload;

        const result = await runAnalysisChain(geminiPayload, {
            geminiKeys:      SHARED_AI.geminiKeys,
            geminiModel:     SHARED_AI.geminiModel,
            openrouterKey:   SHARED_AI.openrouterKey,
            openrouterModel: SHARED_AI.openrouterModel,
            openaiKey:       SHARED_AI.openaiKey,
            openaiModel:     SHARED_AI.openaiModel,
        });

        let responseData = await _injectCostMetadata(result.responseData, result.model);
        responseData._engine    = `${result.engine}-shared`;
        responseData._provider  = result.engine;
        responseData._model     = result.model;
        responseData._via_proxy = true;

        // Log usage (counts toward daily quota + flagged via_proxy=1)
        try {
            const fileType = (bodyFileType || _detectFileTypeFromPayload(geminiPayload) || 'jpg').toLowerCase();
            desktopLink.logUsage({
                userId:        user.id,
                plan:          user.plan,
                fileType,
                fileName:      bodyFileName || null,
                fileSize:      null,
                deviceTokenId: req.deviceTokenId,
                viaProxy:      true,
            ...
            console.warn('[ai/proxy/analyze] log usage failed:', logErr.message);
        }

        return res.json(responseData);
    } catch (err) {
        console.error('[ai/proxy/analyze] error:', err.message);
        ...
        return res.status(status).json({
            ...
            code,
            friendly: 'AI ของบริษัทตอบกลับ error — ลองอีกครั้งหรือสลับมาใช้ API Key ของตัวเอง',
        });
    }
});

/** POST /api/ai/proxy/chat — Material consultation chat using shared keys.
 *  Body: { messages, domain, system } */
app.post('/api/ai/proxy/chat', requireDeviceToken, async (req, res) => {
    const ctx = _proxyPreflight(req, res);
    if (!ctx) return;
    const { user } = ctx;

    try {
        const { messages, domain, system: customSystem } = req.body || {};
        if (!Array.isArray(messages) || messages.length === 0) {
        ...
        const sanitized = messages
            .map(m => ({ role: m.role === 'assistant' ? 'assistant' : 'user', content: String(m.content || '').slice(0, 4000) }))
            .filter(m => m.content.length > 0);
        const system = customSystem || buildSystemPrompt(domain);
        ...
        if (String(domain || '').toLowerCase() === 'density') {
            ...
            if (matches && matches.length > 0) {
                ...
                const display = normalizeMaterialDisplay(m, text);
                const kg_m3 = Number(m.density_kg_m3);
                const reply = formatDensityReply(display, kg_m3, kg_m3 / 1000);
                return res.json({ ok: true, provider: 'local', text: reply });
            }
        }

        // Try Gemini → OpenAI fallback (no vision needed for chat; OpenRouter skipped for chat)
        let text = null, engine = null, model = null;
        if (SHARED_AI.geminiKeys.length > 0) {
            try {
                const contents = sanitized.map(m => ({
                    role: m.role === 'assistant' ? 'model' : 'user',
                    parts: [{ text: m.content }]
                ...
                const key = SHARED_AI.geminiKeys[0];
                const m = SHARED_AI.geminiModel;
                const resp = await axios.post(
                ...
                engine = 'gemini'; model = m;
            } catch (err) {
                console.warn('[proxy/chat] gemini failed → openai fallback:', err.message);
            }
        }
        if (!text && SHARED_AI.openaiKey) {
            const result = await callOpenAIChat(system, sanitized, SHARED_AI.openaiModel, SHARED_AI.openaiKey);
            text = result.text; engine = 'openai'; model = SHARED_AI.openaiModel;
        }
        if (!text) return res.status(503).json({ ok: false, error: 'all_chat_providers_failed' });

        return res.json({ ok: true, provider: engine, model, text, _via_proxy: true });
    } catch (err) {
        console.error('[ai/proxy/chat] error:', err.message);
        return res.status(500).json({ ok: false, error: err.message || 'chat proxy failed' });
    }
});

function _detectFileTypeFromPayload(geminiPayload) {
    try {
        const parts = geminiPayload?.contents?.[0]?.parts || [];
        const imgPart = parts.find(p => p.inlineData);
        const mime = String(imgPart?.inlineData?.mimeType || '').toLowerCase();
        if (mime.includes('pdf')) return 'pdf';
        if (mime.includes('jpeg') || mime.includes('jpg')) return 'jpg';
        if (mime.includes('png')) return 'jpg';  // bucket png with jpg for quota
        return null;
    } catch { return null; }
}

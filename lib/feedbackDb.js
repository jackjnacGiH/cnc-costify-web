...
function _initSchema(db) {
    ...
    _addColumnIfMissing(db, 'licenses', 'valid_from', 'INTEGER');
    _addColumnIfMissing(db, 'licenses', 'valid_until', 'INTEGER');
    _addColumnIfMissing(db, 'licenses', 'revoked', 'INTEGER NOT NULL DEFAULT 0');
    _addColumnIfMissing(db, 'licenses', 'revoked_reason', 'TEXT');

    // V5.9: Plan-based AI proxy — flag rows that used shared server-side AI keys
    _addColumnIfMissing(db, 'usage_log', 'via_proxy', 'INTEGER NOT NULL DEFAULT 0');
    _addColumnIfMissing(db, 'usage_log', 'engine', 'TEXT');     // 'gemini'|'openrouter'|'openai'
    _addColumnIfMissing(db, 'usage_log', 'model', 'TEXT');      // model name actually used
}

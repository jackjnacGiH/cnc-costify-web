 ...
 *
 * V5.9: Added optional `viaProxy`, `engine`, `model` fields to flag rows that
 * consumed company-side AI keys (for cost monitoring).
 */
function logUsage({ userId, plan, fileType, fileName, fileSize, deviceTokenId, viaProxy = false, engine = null, model = null }) {
    const db = getDb();
    const ft = String(fileType || '').toLowerCase();
    if (!['step', 'pdf', 'jpg', 'jpeg', 'png'].includes(ft)) throw new Error('invalid_file_type');
    const dayKey = getThDayKey();
    db.prepare(`
        INSERT INTO usage_log (user_id, day_key, file_type, file_name, file_size, device_token_id, created_at, via_proxy, engine, model)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(
        userId, dayKey,
        ft === 'jpeg' ? 'jpg' : ft,
...
// ─── V5.9: Proxy soft-cap for Yearly/Lifetime ───────────────────────────────
//
// Unlimited plans (yearly/lifetime) have null PLAN_LIMITS so local quota never
// blocks. But when they use the SHARED AI proxy (company pays tokens) we add a
// safety cap to prevent runaway cost. Counts only rows where via_proxy=1.

const PROXY_DAILY_CAPS = {
    free:     0,            // free users cannot use proxy
    monthly:  null,         // monthly is gated by PLAN_LIMITS (30/day) — no extra cap
    yearly:   Number(process.env.PROXY_YEARLY_DAILY_CAP   || 200),
    lifetime: Number(process.env.PROXY_LIFETIME_DAILY_CAP || 300),
};

/**
 * Check whether a user can perform one more proxy-backed AI request today.
 * Returns { allowed: bool, cap: number|null, used: number, remaining: number }.
 *
 * Monthly users fall through to PLAN_LIMITS (no extra cap here).
 * Yearly/Lifetime users hit the soft cap from env vars.
 */
function checkProxyCapStatus(userId, plan) {
    const cap = PROXY_DAILY_CAPS[plan];
    if (cap === null) return { allowed: true, cap: null, used: 0, remaining: -1 };
    if (cap === 0)    return { allowed: false, cap: 0, used: 0, remaining: 0, reason: 'plan_not_eligible' };
    const db = getDb();
    const dayKey = getThDayKey();
    const used = db.prepare(
        'SELECT COUNT(*) AS c FROM usage_log WHERE user_id = ? AND day_key = ? AND via_proxy = 1'
    ).get(userId, dayKey).c;
    const remaining = Math.max(0, cap - used);
    return { allowed: remaining > 0, cap, used, remaining, resetInMs: msUntilThMidnight() };
}

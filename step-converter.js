...
class StepConverter {
    ...
    calculateStockFromSTEP(stepContent) {
        ...
        // Filter outlier points (SolidWorks/CAD STEP files embed annotation/reference points
        // at huge distances; use IQR-based outlier removal on each axis)
        function filterOutliers(arr) {
            ...
            const iqr = q3 - q1;
            // Allow 3× IQR fence; if IQR=0 (flat axis) fall back to ±50× median
            const lo = iqr > 0 ? q1 - 3 * iqr : q1 - Math.abs(q1) * 50 - 1e-3;
            const hi = iqr > 0 ? q3 + 3 * iqr : q3 + Math.abs(q3) * 50 + 1e-3;
            const filtered = arr.filter(v => v >= lo && v <= hi);
            return filtered.length >= 2 ? filtered : arr;
        }
        const allXs = coordinates.map(p => p.x);
        const allYs = coordinates.map(p => p.y);
        const allZs = coordinates.map(p => p.z);
        const xs = filterOutliers(allXs);
        const ys = filterOutliers(allYs);
        const zs = filterOutliers(allZs);

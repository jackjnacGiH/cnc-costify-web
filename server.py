...
if getattr(sys, 'frozen', False):
    ...
    # Add base_dir to DLL search path (Python 3.8+) and PATH
    try:
        os.add_dll_directory(base_dir)
    except (AttributeError, OSError):
        pass
    os.environ['PATH'] = base_dir + os.pathsep + os.environ['PATH']
    logging.info(f"Added {base_dir} to DLL search path and PATH")
...
# Persistent calibration reference path — search bundled (_MEIPASS) and external locations
def _resolve_calibration_path():
    candidates = [
        os.path.join(BASE_DIR, 'calibration_reference.json'),
        os.path.join(os.path.dirname(__file__), 'calibration_reference.json'),
    ]
    for p in candidates:
        if os.path.exists(p):
            return p
...
# Prefer LocalAppData (always writable); fallback to ProgramData; last resort next to script
def _pick_config_dir():
    """Pick config dir, preferring an EXISTING api_config.json with non-empty keys
    so Python doesn't ignore configs the Node frontend wrote elsewhere."""
    try:
    ...
    _candidates = [
        os.path.join(_localappdata, 'CNC Costify AI', 'config'),
        os.path.join(os.environ.get('PROGRAMDATA', r'C:\ProgramData'), 'CNC Costify AI', 'config'),
        os.path.join(os.path.dirname(__file__), 'config'),
    ]
    # Pass 1: existing config with non-empty Gemini key or OpenRouter key
    for _d in _candidates:
        _cfg = os.path.join(_d, 'api_config.json')
        if os.path.exists(_cfg):
            try:
                with open(_cfg, 'r', encoding='utf-8') as _f:
                    _data = json.load(_f) or {}
                _g = (_data.get('gemini') or {})
                _or = (_data.get('openrouter') or {})
                _has_keys = bool(_g.get('api_key') or _g.get('api_keys') or _or.get('api_key'))
                if _has_keys:
                    return _d
            except Exception:
                pass
    # Pass 2: any existing config (even if empty)
    for _d in _candidates:
        if os.path.exists(os.path.join(_d, 'api_config.json')):
            return _d
    # Pass 3: first writable candidate
    for _d in _candidates:
        try:
            os.makedirs(_d, exist_ok=True)
            _t = os.path.join(_d, '.write_test')
...
try:
    STEP_TIMEOUT_SECS = int(os.environ.get('STEP_TIMEOUT_SECS') or '60')
except Exception:
    STEP_TIMEOUT_SECS = 60

def _run_with_timeout(fn, *args, timeout=None):
    """Run fn(*args) in a thread pool with a timeout (seconds).
    Raises TimeoutError with a Thai-friendly message if exceeded."""
    t = timeout if timeout is not None else STEP_TIMEOUT_SECS
    with concurrent.futures.ThreadPoolExecutor(max_workers=1) as executor:
        ...
        try:
            ...
            raise TimeoutError(
                f'การประมวลผลไฟล์ STEP ใช้เวลาเกิน {t} วินาที — '
                f'ไฟล์อาจมีรูปทรงซับซ้อนเกินไป กรุณาลองไฟล์ที่เล็กหรือเรียบกว่า'
            )
...
def _load_ai_config() -> dict:
    ...
    provider = cfg.get('provider') or os.environ.get('AI_PROVIDER') or 'gemini'
    ...
    cfg = {
        ...
        'gemini': {
            ...
            # Preserve multi-key array (managed by Node server) — fall back to single key
            'api_keys': gemini.get('api_keys') if isinstance(gemini.get('api_keys'), list) else None,
            'model': gemini.get('model') or os.environ.get('GEMINI_MODEL') or 'gemini-1.5-flash'
        },
        'openrouter': {
            'api_key': openrouter.get('api_key') or '',
            'model': openrouter.get('model') or 'qwen/qwen2.5-vl-72b-instruct',
        },
    }
    return cfg

def _get_gemini_keys(cfg: dict) -> list:
    """Return all configured Gemini API keys (api_keys array or single api_key)."""
    g = cfg.get('gemini') or {}
    keys = g.get('api_keys') if isinstance(g.get('api_keys'), list) else None
    if keys:
        return [k for k in keys if k and k.strip()]
    single = (g.get('api_key') or os.environ.get('GOOGLE_API_KEY') or os.environ.get('GEMINI_API_KEY') or '').strip()
    return [single] if single else []

def _save_ai_config(cfg: dict) -> None:
    try:
        # Preserve multi-key array + OpenRouter section (managed by Node server)
        g_in = cfg.get('gemini') or {}
        or_in = cfg.get('openrouter') or {}
        gemini_out = {
            'api_key': g_in.get('api_key') or '',
            'model': g_in.get('model') or 'gemini-1.5-flash'
        }
        if isinstance(g_in.get('api_keys'), list):
            gemini_out['api_keys'] = [k for k in g_in['api_keys'] if k]
        sanitized = {
            'provider': cfg.get('provider'),
            'gemini': gemini_out,
        }
        if or_in.get('api_key') or or_in.get('model'):
            sanitized['openrouter'] = {
                'api_key': or_in.get('api_key') or '',
                'model': or_in.get('model') or 'qwen/qwen2.5-vl-72b-instruct',
            }
        try:
...
def _refresh_env_from_cfg(cfg: dict) -> None:
    try:
        os.environ['AI_PROVIDER'] = str(cfg.get('provider') or 'gemini')
...
# Test actual write access before committing to a log directory
def _pick_log_dir():
    _programdata = os.environ.get('PROGRAMDATA', r'C:\ProgramData')
    _localappdata = os.environ.get('LOCALAPPDATA', os.path.expanduser('~'))
    # LOCALAPPDATA first — always writable by current user without admin rights
    _candidates = [
        os.path.join(_localappdata, 'CNC Costify AI', 'logs'),
        os.path.join(_programdata,  'CNC Costify AI', 'logs'),
        os.path.join(os.path.dirname(__file__), 'logs'),
    ]
    for _d in _candidates:
        try:
            os.makedirs(_d, exist_ok=True)
            _test = os.path.join(_d, 'server.log')
            with open(_test, 'a', encoding='utf-8') as _f:
                pass
            return _d
        except Exception:
            continue
    return _candidates[-1]  # last-resort (may still fail, but try)
...
try:
    ...
    # Log setup failed (e.g. permission denied) — continue without file logging
    import sys as _sys
    print(f"[WARN] File logging disabled: {_log_err}", file=_sys.stderr)
...
# --- STT (Speech-to-Text) — disabled (OpenAI Whisper removed) ---
...
def api_stt_transcribe():
    # OpenAI Whisper removed — return 501 Not Implemented
    from flask import Response
    if request.method == 'OPTIONS':
        ...
        try:
            ...
            pass
        return resp
    return jsonify({'ok': False, 'error': 'Speech-to-text is not configured (OpenAI Whisper removed).'}), 501
...
def _build_system_prompt(domain: str) -> str:
    """Domain-aware system prompt. ALL responses must be SHORT & CONCISE
    (3-6 bullet items max, ≤120 words). Avoid long paragraphs and intros."""
    d = str(domain or '').lower().strip()
    base_rules = (
        ' ตอบให้สั้น กระชับ ไม่เกิน 6 ข้อ/100 คำ '
        'เป็น bullet สั้นๆ ไม่ต้องอารัมภบท ห้ามเขียนคำขอบคุณหรือแนะนำตัวเอง '
        'ตอบเป็นภาษาไทยเสมอ ใช้ตัวเลข/ค่าจริง เมื่อเป็นไปได้'
    )
    prompts = {
        'density':       'ตอบ Density ของวัสดุ หน่วย g/cm³ + ตัวอย่างใช้งาน 1-2 ข้อ',
        'spec':          'สเปควัสดุ: ส่วนผสม, มาตรฐานเทียบเท่า (ASTM/JIS/DIN), Hardness',
        'substitute':    'แนะนำวัสดุทดแทน 2-3 ตัว + เหตุผลสั้นๆ',
        'hardening':     'การชุบแข็ง: วิธี, อุณหภูมิ, HRC ที่ได้, ข้อควรระวัง',
        'surface':       'การทำผิว: ตัวเลือก, ความหนา, ราคาประมาณการ',
        'machining':     'พารามิเตอร์ตัดเฉือน: Speed, Feed, DOC สำหรับ Endmill/Drill',
        'heat':          'ความทนความร้อน: อุณหภูมิสูงสุด + วัสดุเทียบเคียง',
        'corrosion':     'การกัดกร่อน: เกรด/coating ที่ทน + สภาพแวดล้อม',
        'mechanical':    'คุณสมบัติทางกล: Tensile, Yield, Elongation, Hardness',
        'welding':       'การเชื่อม: วิธีที่เหมาะ, เกรดลวดเชื่อม, Pre/post heat',
        'coating':       'Coating: เลือก Anodize/Hard/Black Oxide ตามการใช้งาน',
        'tolerance':     'ความเที่ยง/Tolerance ตามเกรด ISO 2768 หรือ Class fit',
        'general':       'ปรึกษาทั่วไปเรื่องวิศวกรรม/CNC',
    }
    persona = prompts.get(d) if d in prompts else None
    if not persona:
        # legacy aliases
        if 'วัสดุ' in d or d in ('material', 'สเปควัตถุดิบ'):
            persona = prompts['spec']
        else:
            persona = prompts['general']
    return f'คุณคือผู้ช่วยด้านงาน CNC. {persona}.{base_rules}'
...
def _call_openrouter_chat(messages: list, system_text: str, api_key: str, model: str) -> str:
    """Call OpenRouter chat completions API. Returns assistant text or raises."""
    ...
    or_messages = [{'role': 'system', 'content': system_text}] if system_text else []
    for m in messages:
        role = m.get('role') or 'user'
        if role == 'assistant':
            or_messages.append({'role': 'assistant', 'content': m.get('content') or ''})
        else:
            or_messages.append({'role': 'user', 'content': m.get('content') or ''})
    payload = {
        'model': model,
    ...
    code, text = _http_post(url, headers=headers, json_body=payload, timeout=GEMINI_TIMEOUT_SECS, retries=1)
    obj = json.loads(text)
    if isinstance(obj, dict) and obj.get('error'):
        raise RuntimeError(f"OpenRouter error: {obj['error'].get('message') or obj['error']}")
...
def api_chat_post():
    try:
        ...
        gemini_keys = _get_gemini_keys(cfg)
        or_cfg = cfg.get('openrouter') or {}
        or_key = (or_cfg.get('api_key') or '').strip()
        or_model = or_cfg.get('model') or 'qwen/qwen2.5-vl-72b-instruct'
        gemini_model = (cfg.get('gemini') or {}).get('model') or 'gemini-1.5-flash'

        # Direct OpenRouter request (provider='openrouter')
        if provider in ('openrouter', 'or'):
            if not or_key:
                return jsonify({'ok': False, 'error': 'OpenRouter API key ไม่ถูกตั้งค่า — กรุณาไปที่ Settings'}), 400
            try:
                text = _call_openrouter_chat(messages, system_text, or_key, or_model)
                return jsonify({'ok': True, 'text': text, '_provider': 'openrouter', '_model': or_model})
            except Exception as e:
                return jsonify({'ok': False, 'error': f'OpenRouter: {e}'}), 500

        # Default path: try Gemini first (rotate through all keys), then OpenRouter fallback
        if provider in ('gemini', 'google'):
            if not gemini_keys and not or_key:
                return jsonify({'ok': False,
                                'error': 'ยังไม่ได้ตั้งค่า API key — กรุณาไปที่ Settings → ตั้งค่า API'}), 400

            api_base = 'https://generativelanguage.googleapis.com'
            contents = _coerce_messages_for_gemini(messages, system_text)
            last_gemini_err = None

            # Generation config tuned for SHORT + FAST chat responses.
            # gemini-2.5-* has built-in "thinking" that eats tokens before output;
            # set thinkingBudget=0 to skip it. maxOutputTokens covers final answer.
            gen_config = {
                'temperature': 0.4,
                'topP': 0.85,
            ...
            for key_idx, api_key in enumerate(gemini_keys):
                def _gen(ver: str, key=api_key):
                    url = f"{api_base}/{ver}/models/{gemini_model}:generateContent?key={key}"
                    return _http_post(url, headers={},
                                      json_body={'contents': contents, 'generationConfig': gen_config},
                                      timeout=GEMINI_TIMEOUT_SECS, retries=1)
                try:
                    ...
                    try:
                        ...
                        status, code_ = '', getattr(he, 'code', 0)
                    if status == 'NOT_FOUND' or code_ == 404:
                        try:
                            code, text = _gen('v1')
                        except Exception as e2:
                            last_gemini_err = f'Gemini key {key_idx+1}: {e2}'
                            continue
                    elif code_ in (429, 403, 400):  # quota/auth/bad-request → try next key
                        last_gemini_err = f'Gemini key {key_idx+1} HTTP {code_}: {body[:120]}'
                        continue
                    else:
                        last_gemini_err = body or str(he)
                        break
                except urllib.error.URLError as ue:
                    msg = str(getattr(ue, 'reason', ue))
                    last_gemini_err = f'Gemini network: {msg}'
                    if 'timed out' in msg.lower():
                        continue  # try next key
                    break
                except Exception as e:
                    last_gemini_err = f'Gemini: {e}'
                    break
                ...
                if content:
                    return jsonify({'ok': True, 'text': content, '_provider': 'gemini',
                                    '_model': gemini_model, '_key_index': key_idx + 1})
                last_gemini_err = 'Gemini returned empty response'

            # All Gemini keys exhausted (or none) → try OpenRouter fallback
            if or_key:
                try:
                    app.logger.warning(f'Gemini failed ({last_gemini_err}) → OpenRouter fallback ({or_model})')
                except Exception:
                    pass
                try:
                    text = _call_openrouter_chat(messages, system_text, or_key, or_model)
                    return jsonify({'ok': True, 'text': text, '_provider': 'openrouter',
                                    '_model': or_model, '_fallback_reason': str(last_gemini_err)[:200]})
                except Exception as e:
                    return jsonify({'ok': False,
                                    'error': f'Gemini ใช้ไม่ได้ และ OpenRouter ก็ล้มเหลว: {e}'}), 500
            # No fallback configured
            return jsonify({'ok': False,
                            'error': f'Gemini ไม่ตอบสนอง: {last_gemini_err or "unknown"}. แนะนำตั้งค่า OpenRouter Fallback'}), 500
...
def _load_and_clean_step_shape(filepath):
    ...
    try:
        ...
        bbox = Bnd_Box()
        # Try function-form first (older OCC), fall back to class method (newer OCC)
        try:
...
def compute_step_stock_material(filepath, prefer_obb: bool = True):
    ...
    if cylinder_radius is not None and cylinder_axis_dir is not None:
        ...
        # Validate cylinder: Use Volume Ratio (BVR) + Area Fraction + Cross-section
        part_volume = 0.0
        try:
            ...
            _vprops = GProp_GProps()
            brepgprop_VolumeProperties(shape, _vprops)
            part_volume = _vprops.Mass()
        except Exception:
            pass

        bb_volume = (aabb_dx * aabb_dy * aabb_dz)
        if prefer_obb and obb_available and obb_dims:
            ob_v = obb_dims[0] * obb_dims[1] * obb_dims[2]
            if ob_v > 0: bb_volume = ob_v
        
        bvr = 1.0
        if bb_volume > 0 and part_volume > 0:
            bvr = part_volume / bb_volume
        ...
        # 1. Bounding Volume Ratio (BVR) check to prevent Box with fillets being misclassified
        is_cylinder = False
        
        # Rule 1: Solid Cylinder (BVR ~ 0.785, area_frac > 0.25)
        if valid_perp and area_frac >= 0.25 and bvr <= 0.90:
            is_cylinder = True
            
        # Rule 2: Hollow Cylinder/Pipe (BVR is very low, e.g. < 0.6, area_frac > 0.15)
        elif valid_perp and bvr <= 0.60 and area_frac >= 0.15:
            is_cylinder = True
            
        # Rule 3: Very clear cylinder surface (> 40%) even if valid_perp fails slightly
        elif area_frac >= 0.40:
            is_cylinder = True
            
        # Rule 4: Standard fallback (Original logic)
        elif valid_perp and area_frac >= 0.30:
            is_cylinder = True
        ...
        if is_cylinder:
            ...
            try:
                ...
                dbg['bvr_ratio'] = round(bvr, 3)
                dbg['cylinder_area_frac'] = round(area_frac, 3)
            ...
            try:
                ...
                # Check if two dimensions are perfectly or almost equal (within 0.5 mm or 5%)
                print('CHECKING DIFF')
                diff = abs(srt[1] - srt[0])
                if diff <= 0.5 or (dia_guess > 0 and diff / dia_guess <= 0.05):
    ...
    # Global Override: If 2 dimensions are nearly equal, FORCE cylinder
    if result.get('type') == 'box':
        w_ = result['stock']['width_mm']
        d_ = result['stock']['depth_mm']
        h_ = result['stock']['height_mm']
        srt = sorted([w_, d_, h_])
        dia_guess = (srt[0] + srt[1]) * 0.5
        len_guess = srt[2]
        diff = abs(srt[1] - srt[0])
...
def api_step_volume():
    ...
    try:
        with tempfile.NamedTemporaryFile(delete=False, suffix='.step') as tmp:
            file.save(tmp.name)
            tmp_path = tmp.name
        volume = _run_with_timeout(compute_step_volume, tmp_path)
        ...
        app.logger.warning('api_step_volume timeout: %s', te)
...
def api_step_area():
    ...
    try:
        with tempfile.NamedTemporaryFile(delete=False, suffix='.step') as tmp:
            file.save(tmp.name)
            tmp_path = tmp.name
        area = _run_with_timeout(compute_step_area, tmp_path)
        runtime_ms = int((time.time() - start) * 1000)
        return jsonify({
            'file': file.filename,
            'surface_area_mm2': area,
        ...
        app.logger.warning('api_step_area timeout: %s', te)
        return jsonify({'error': str(te), 'timeout': True}), 504
    except Exception as e:
...
def api_step_mprops():
    ...
    try:
        with tempfile.NamedTemporaryFile(delete=False, suffix='.step') as tmp:
            file.save(tmp.name)
            tmp_path = tmp.name
        props = _run_with_timeout(compute_step_mass_properties, tmp_path)
        runtime_ms = int((time.time() - start) * 1000)
        props['file'] = file.filename
        props['runtime_ms'] = runtime_ms
        return jsonify(props)
    except TimeoutError as te:
        app.logger.warning('api_step_mprops timeout: %s', te)
        return jsonify({'error': str(te), 'timeout': True}), 504
    except Exception as e:
...
def api_step_stock():
    ...
    try:
        with tempfile.NamedTemporaryFile(delete=False, suffix='.step') as tmp:
            file.save(tmp.name)
            tmp_path = tmp.name
        stock = _run_with_timeout(compute_step_stock_material, tmp_path)
        runtime_ms = int((time.time() - start) * 1000)
        stock['file'] = file.filename
        stock['runtime_ms'] = runtime_ms
        # Log returned stock dimensions for debugging
        try:
            s = stock.get('stock', {})
            t = stock.get('type', '?')
            if t == 'box':
                logging.info(f"Stock result [{file.filename}]: {t} w={s.get('width_mm')} d={s.get('depth_mm')} h={s.get('height_mm')} vol={stock.get('volume_mm3')}")
            elif t == 'cylinder':
                logging.info(f"Stock result [{file.filename}]: {t} dia={s.get('diameter_mm')} len={s.get('length_mm')} vol={stock.get('volume_mm3')}")
        except Exception:
            pass
        return jsonify(stock)
    except TimeoutError as te:
        app.logger.warning('api_step_stock timeout: %s', te)
        return jsonify({'error': str(te), 'timeout': True}), 504
    except Exception as e:

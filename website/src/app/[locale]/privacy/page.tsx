...
export default async function PrivacyPage({ params }: Props) {
  ...
  return (
    <>
      ...
      {/* ── Hero ───────────────────────────────────────────────── */}
      <section className="site-hero pt-16 pb-12 md:pt-24 md:pb-16">
        <div className="site-hero-orb" />

        <div className="relative max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <div className="site-eyebrow mb-6">
            <Shield size={16} className="text-emerald-300" />
            {isTh ? "นโยบายความเป็นส่วนตัว" : "Privacy Policy"}
          </div>

          <h1 className="site-hero-title text-3xl md:text-5xl font-black tracking-tight mb-4">
          ...
          <p className="site-hero-copy text-base md:text-lg max-w-2xl mx-auto">
            {isTh
          ...
          <div className="mt-6 inline-flex items-center gap-2 text-xs text-slate-600 bg-white/80 px-3 py-1.5 rounded-full border border-slate-200 backdrop-blur">
            <RefreshCw size={12} />
            {isTh ? "ปรับปรุงล่าสุด:" : "Last updated:"} <span className="font-bold text-slate-800">{lastUpdated}</span>
          </div>
        </div>
      </section>
